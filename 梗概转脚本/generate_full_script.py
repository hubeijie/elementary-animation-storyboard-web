...
def build_prompt(title: str, synopsis: str, teaching: list[dict]) -> str:
    spec = """【脚本规范 · elementary-animation-storyboard-script】
1）输出一个 Markdown 表格，五列且顺序固定：镜号 | 画面 | 台词 | 画面描述 | 备注。
2）第一行表头必须是：| 镜号 | 画面 | 台词 | 画面描述 | 备注 |
3）第二行必须是分隔行：| --- | --- | --- | --- | --- |
4）台词与画面描述必须用相同编号 【1】【2】【3】… 逐步对齐（可含 【2.5】 等与教研一致）。
5）画面描述可写 *复用场景-xxx* *复用道具-xxx* *复用人设-xxx*。
6）故事梗概是剧情主体：按时间顺序拆成多镜；不要把梗概单独堆在表前几行当「参考区」。
7）教研 JSON 每一行：将「逐字稿」融入「台词」，「动效」融入「画面描述」，保留【n】与可执行动效细节；「画面」列可作环节标签。
8）除表格外不要输出任何解释文字。
"""
    return (
        spec
